...
- ไอคอนถ้วยกาแฟ เปลี่ยนเป็นสีเขียวเมื่อกำลังทำงาน (หรือสีธีมที่เลือกไว้)
- เลือกสีธีมได้ 6 สี (เขียว / ฟ้า / ม่วง / ส้ม / ชมพู / ฟ้าอมเขียว) — จำค่าไว้ให้ครั้งต่อไป
- อัปเดตในตัว: ปุ่ม "ตรวจสอบอัปเดต" ที่มุมล่างขวา — เจอเวอร์ชันใหม่แล้วกดปุ่มเดียว ดาวน์โหลด ติดตั้ง และเปิดแอปใหม่ให้เอง (ไม่ต้องใส่รหัสผ่าน ถ้าแอปอยู่ใน `~/Applications`)
...
### ติดตั้งง่ายกว่า: ไฟล์ .pkg หรือ .dmg

ถ้าไม่อยากคอมไพล์เอง โหลดจากหน้า [Releases](../../releases) แล้วเลือกแบบใดแบบหนึ่ง:

**`.pkg` (ง่ายสุด แนะนำ)** — ดับเบิลคลิกแล้วกด Continue/Install ตามขั้นตอน ติดตั้งลง `/Applications` ให้เอง พร้อมล้าง quarantine ให้อัตโนมัติ ไม่ต้องคลิกขวา > Open

**`.dmg` (แบบลากวาง)** — ดับเบิลคลิกไฟล์ แล้วลากไอคอน KeepAwake ไปวางที่โฟลเดอร์ Applications (ครั้งแรกถ้าโดนเตือนเรื่อง unidentified developer: คลิกขวา > Open)

สร้างไฟล์ติดตั้งเองได้ด้วย `./build-pkg.sh` หรือ `./build-dmg.sh` (ต้อง build หรือมี `KeepAwake.app` อยู่ก่อน)
